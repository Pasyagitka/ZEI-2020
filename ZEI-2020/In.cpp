		...
		for (int i = 0; in.text[i] != IN_CODE_N; i++)
		{
			...
			if (in.text[i] == IN_CODE_SPACE) {
				switch (in.text[i + 1]) {
					...
					case IN_CODE_LESS:  case IN_CODE_MORE: case IN_CODE_INEQUALITY:  case IN_CODE_ASSIGN: case IN_CODE_LEFTSHIFT: case IN_CODE_RIGHTSHIFT:  case IN_CODE_LEFTBRACE: case IN_CODE_RIGHTBRACE: case IN_CODE_OPENBRACKET: case IN_CODE_CLOSEBRACKET: case IN_CODE_EUALITY: case IN_CODE_PLUS: case IN_CODE_MINUS: case IN_CODE_DIVISION: case IN_CODE_STAR: {
						in.size--;  continue;
					}
				}
			}
